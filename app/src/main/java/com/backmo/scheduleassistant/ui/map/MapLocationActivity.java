...
public class MapLocationActivity extends AppCompatActivity {
    ...
    private void loadMapHtmlWithBaseUrl() {
        try {
            java.io.BufferedReader br = new java.io.BufferedReader(new java.io.InputStreamReader(getAssets().open("map.html"), java.nio.charset.StandardCharsets.UTF_8));
            StringBuilder sb = new StringBuilder();
            String line;
            while ((line = br.readLine()) != null) sb.append(line).append('\n');
            br.close();
            webView.loadDataWithBaseURL("https://scheduleassistant.local/", sb.toString(), "text/html", "UTF-8", null);
        } catch (Exception e) {
            webView.loadUrl("file:///android_asset/map.html");
        }
    }
    ...
    protected void onResume() {
        super.onResume();
        // WebView 无需额外 onResume 处理
    }

    @Override
    protected void onPause() {
        super.onPause();
        // WebView 无需额外 onPause 处理
    }
